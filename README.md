...
- **6 类物理拟真音效已按 App 逐个接线**（`audioEngine.playSfx`）：盖印 / 撕票 / 翻页 / 落针 / 卡带 / 星音，
  共 10 个调用点覆盖 `memoir`（工坊）/ `constellation`（点星，按评分 432~528Hz 定频）/ `standby`（唱臂落针）。
  **音色相对 App 做了悦耳化**：App 原波形是裸正弦且无起音包络（听感是电子哔声 + 音头爆音），
  此处改为噪声整形 / 共振峰 / 钟式泛音堆 + 3ms 淡入 + 软限幅，并把峰值压回 App 量级。
